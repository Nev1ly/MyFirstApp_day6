...
class PostDaoImpl(private val db: SQLiteDatabase) : PostDao {
    ...
    override fun insert(post: Post): Post {
        val values = postToContentValues(post).apply {
            remove(BaseColumns._ID) // ID сгенерируется автоматически
        }
    ...
    override fun update(post: Post): Post {
        val values = postToContentValues(post).apply {
            remove(BaseColumns._ID) // ID не обновляем
        }
    ...
    override fun shareById(id: Long) {
        ...
        db.execSQL(
            "UPDATE ${PostContract.TABLE_NAME} SET ${Columns.SHARES} = ${Columns.SHARES} + 1 WHERE ${BaseColumns._ID} = ?",
            arrayOf(id)
        )
    }
    override fun increaseViews(id: Long) {
        db.execSQL(
            "UPDATE ${PostContract.TABLE_NAME} SET ${Columns.VIEWS} = ${Columns.VIEWS} + 1 WHERE ${BaseColumns._ID} = ?",
            arrayOf(id)
        )
    }
    // Маппинг: Cursor -> Post
